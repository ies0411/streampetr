...
def get_abs_pos(abs_pos, has_cls_token, hw):
    ...
    if size != h or size != w:
        ...
        new_abs_pos = F.interpolate(
            abs_pos.reshape(1, size, size, -1)
            .permute(0, 3, 1, 2)
            .float(),  # bf16 is not implemented
            size=(h, w),
            mode="bicubic",
            align_corners=False,
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash_attn:
            ...
            x, attn_weights = self.inner_attn(
                q, kv, key_padding_mask=None, causal=False
            )
    ...
    def __init__(
        ...
        for i in range(depth):
            block = Block(
                ...
                norm_layer=norm_layer,
                window_size=(
                    window_size if i in window_block_indexes else global_window_size
                ),
